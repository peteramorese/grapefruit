...
namespace YAML {
    template <>
    struct convert<GF::FormalMethods::Alphabet> {
        static Node encode(const GF::FormalMethods::Alphabet& alphabet) {
            Node node;
            for (auto letter : alphabet) node.push_back(letter);
            return node;
        }
        static bool decode(const Node& node, GF::FormalMethods::Alphabet& alphabet) {
    ...
    template <>
    struct convert<std::set<GF::Node>> {
        static Node encode(const std::set<GF::Node>& state_set) {
            Node node;
            for (auto state : state_set) node.push_back(state);
            return node;
        }
        static bool decode(const Node& node, std::set<GF::Node>& state_set) {
            if (!node.IsSequence()) return false;
...
static GF::FormalMethods::Alphabet operator+(GF::FormalMethods::Alphabet alph_1, GF::FormalMethods::Alphabet alph_2) {
    alph_1.merge(alph_2);
    return alph_1;
}
