...
namespace std {
	template <>
	struct hash<GF::DiscreteModel::State> {
		std::size_t operator()(const GF::DiscreteModel::State& state) const {
			std::size_t hash = state.m_state_index_buffer[0];
			for (GF::DiscreteModel::dimension_t dim = 0; dim < state.m_ss->rank(); ++dim) {
