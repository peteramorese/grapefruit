...
namespace DiscreteModel {
	...
	const std::string& State::getVar(const std::string& label) const {
		return m_ss->interpretLabel(this, label);
	}

	void State::print() const {
		const Containers::SizedArray<const std::string*> vars = m_ss->interpret(this);
		std::string vars_str = *(vars[0]);
		for (uint8_t i=1; i < m_ss->rank(); ++i) {
			vars_str += ", " + *(vars[i]);
		}
