...
int main(int argc, char* argv[]) {
	...
	auto n_efe_samples = parser.parse<uint32_t>("efe-samples", 100u, "Number of samples used for approximating the expected posterior entropy");
	...
	for (uint32_t trial = 0; trial < n_trials.get(); ++trial) {
		...
		p_ev.mu(0) *= GF::RNG::srandf(0.6, 1.2);
		p_ev.mu(1) *= GF::RNG::srandf(0.8, 1.2);
		...
		Learner<N> prl(behavior_handler, data_collector, n_efe_samples.get(), false);
