...
int main(int argc, char* argv[]) {
	...
	while (trial < n_trials.get()) {
		...
		std::shared_ptr<BehaviorHandlerType> mutable_behavior_handler = std::make_shared<BehaviorHandlerType>(*targets.behavior_handler);
		...
		Learner<N> prl(mutable_behavior_handler, data_collector, n_efe_samples.get(), verbose);
