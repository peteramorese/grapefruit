...
namespace Planner {
    ...
    template <class SYMBOLIC_GRAPH_T, class INHERITED_COST_T, class HEURISTIC_T, class...OBJ_ARGS_T>
    class MOPreferencePlannerSearchProblem {
        public:
            ...
            struct CostVectorActionEdge {
                ...
                CostVectorActionEdge(OBJ_ARGS_T&&...objs, graph_t::edge_t::action_t&& action_) : cv(std::forward<OBJ_ARGS_T>(objs)...), action(action_) {}
        ...
        public: // Methods & members required by any search problem
            ...
            inline std::vector<edge_t> neighborEdges(ProdNode node) const {
                ...
                for (auto& inherited_edge : inherited_edges) {
                    converted_edges.emplace_back(OBJ_ARGS_T(*m_graph, node, std::move(inherited_edge))..., static_cast<action_t&&>(inherited_edge));
                }
        ...
        public:
            MOPreferencePlannerSearchProblem(const std::shared_ptr<SYMBOLIC_GRAPH_T>& sym_graph) : m_graph(sym_graph) {}
